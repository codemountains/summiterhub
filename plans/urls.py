...
# Plan bookmark
plan_bookmarks_router = routers.DefaultRouter()
plan_bookmarks_router.register('', views.BookmarkViewSet)


urlpatterns = [
	path('', include(plan_router.urls)),
	path('<uuid:plan_id>/', include(plan_details_router.urls)),
	path('<uuid:plan_id>/routes/<uuid:plan_route_id>/', include(plan_route_router.urls)),
]
